...
namespace ionet
{
    ...
    void EventLoop::addEvent(int fd,int event,Fdevent::fd_callback cb)
    {
        if(event & Ionet::IONET_READ)
        {
            ...
            并且设置可读,读回调函数
            */
            Fdevent *tmpEvent=new Fdevent(fd);
            tmpEvent->setReadable(true);
        ...
        else if(event & Ionet::IONET_WRITE)
        {
            ...
            */
            map_callback[fd]->setWriteable(true);
            map_callback[fd]->setWriteCb(cb);
            map_callback[fd]->setReadable(false);//写事件先把读事件关闭,写事件回调函数执行之后再打开.
    ...
    void EventLoop::delEvent(int fd,int event)
    {
        ...
        else if(event & Ionet::IONET_WRITE)
        {
            ...
            */
            map_callback[fd]->setWriteable(false);
            epoller->delEvent(fd, event);
    ...
    void EventLoop::updateEvent(int fd)
    {
        //主要是提供开启fd的读事件接口
        map_callback[fd]->setReadable(true);
    }
